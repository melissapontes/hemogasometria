...
export function HamburgerMenu({ onSignOut }: { onSignOut: () => Promise<void> }) {
  ...
  return (
    <>
      ...
      <div className={`fixed inset-y-0 right-0 z-50 flex w-80 max-w-[90vw] flex-col bg-white shadow-xl transition-transform duration-300 ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="px-4 pt-4">
            <p className="mb-2 px-1 text-xs font-semibold uppercase tracking-widest text-slate-400">Conteúdo</p>
            <Link
              className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-[#4d4d4d] hover:bg-slate-50"
              to="/references"
              onClick={() => setIsOpen(false)}
            >
          ...
          {/* Settings */}
          <div className="px-4 pt-4">
            <p className="mb-2 px-1 text-xs font-semibold uppercase tracking-widest text-slate-400">Conta</p>
            <button
              className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-[#4d4d4d] hover:bg-slate-50"
              type="button"
            ...
            {settingsOpen && (
              <div className="mt-3 space-y-4 rounded-xl border border-slate-100 bg-slate-50 px-4 py-4">
                <div>
                  <label className="mb-1 block text-xs font-medium text-[#4d4d4d]" htmlFor="profile-nome">Nome</label>
                  <input
                ...
                <div>
                  <label className="mb-1 block text-xs font-medium text-[#4d4d4d]" htmlFor="profile-crmv">CRMV</label>
                  <input
                    className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-[#4d4d4d] outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-300"
                    id="profile-crmv"
                ...
                <div>
                  <label className="mb-1 block text-xs font-medium text-[#4d4d4d]" htmlFor="profile-estado">Estado</label>
                  <select
                    className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-[#4d4d4d] outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-300"
                    id="profile-estado"
        ...
        <div className="border-t border-slate-200 px-5 py-4 space-y-3">
          <div className="flex justify-center gap-4">
            <Link className="text-xs text-slate-400 hover:text-slate-600 hover:underline" to="/terms" onClick={() => setIsOpen(false)}>
              Termos de Uso
            </Link>
            <Link className="text-xs text-slate-400 hover:text-slate-600 hover:underline" to="/privacy" onClick={() => setIsOpen(false)}>
              Privacidade
            </Link>
          </div>
